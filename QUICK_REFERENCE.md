...
- **Setup** (hardware): 20 rows, **scrolls vertically** (6-row window, `row/total` counter,
  ...
  Stall (collision detect on/off), StallSG (StallGuard sensitivity −64…63).
- **Collision detect**: `Stall` ON + `Chop = Spread` → a blocked/stalled arm parks, disables,
  and shows red **COLLISION** + ERROR (START to recover). Needs SpreadCycle; tune `StallSG`.
- **Settings survive firmware updates**: loading is version-tolerant — new fields default,
  existing values are kept (no more re-entering settings after flashing).
